...
if(isset($_GET['action'])){
 try{
  ...
  if($a==='filters'){ $st=direct_db()->prepare("SELECT DISTINCT crop_season FROM public.direct_sales WHERE sale_category=:c AND COALESCE(crop_season,'')<>'' ORDER BY crop_season DESC");$st->execute(['c'=>$cat]);direct_json(true,'',['seasons'=>$st->fetchAll(PDO::FETCH_COLUMN)]); }
  if($a==='rows'){ $w=['sale_category=:c'];$p=['c'=>$cat];if($season!==''){$w[]='crop_season=:s';$p['s']=$season;} $st=direct_db()->prepare('SELECT * FROM public.direct_sales WHERE '.implode(' AND ',$w).' ORDER BY invoice_date DESC NULLS LAST, invoice_number DESC, id DESC');$st->execute($p);direct_json(true,'',['rows'=>$st->fetchAll()]); }
